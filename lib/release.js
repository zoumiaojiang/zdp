...
function jsUglify(root, fileList) {
    ...
    fileList.forEach( function (item) {
        ...
        if (fs.statSync(fullPath).isFile() 
            ...
            var minCode = utils.jsUglify(fullPath, fs.readFileSync(fullPath, 'utf8'));
            var itemTmp = item.split('/');
            var tempath = '';
            if (itemTmp.length > 1) {
                item = itemTmp[itemTmp.length - 1];
            }
            tempath = path.resolve(utils.getTmpDir(), item + 'tmp');
            fs.writeFileSync(tempath, minCode);
            fs.chmodSync(tempath, 0777);
        }
    });
}
...
function mincss(tplname) {
    ...
    var tempath = dealPath(tplname, 'page.css');
    fs.writeFileSync(tempath, minCode);
}
...
function parse(tplname) {
    ...
    if (regexp.test(_page)) {
        ...
        var tmpDir = jsUglify(dealPath(tplname), filenames);
        ...
        filenames.forEach(function (item) {
            ...
            var confile = extname === '.js' 
                    ? path.resolve(utils.getTmpDir(), tmpItem + 'tmp')
